...
const ChatStory = (() => {
  ...
  function next() {
    ...
    const m=_msgs[_idx]; _idx++;
    // Conditional skip: skipIf=[{key:'you_after_photo', value:'flee'}] skips this message
    // if state[key] === value. Multiple conditions = OR (any match skips).
    if(m.skipIf && Array.isArray(m.skipIf)){
    ...
    // Conditional show: showIf=[{key:'you_after_photo', value:'confront'}] shows only
    // if state[key] === value. Multiple conditions = AND (all must match).
    if(m.showIf && Array.isArray(m.showIf)){
